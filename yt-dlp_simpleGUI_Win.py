...
# Windows version, only things that change here are using yt-dlp.exe instead of ./yt-dlp,
# and leaving some parts commented since they are only for Linux.

# I am using a script for converting this, please check the code for any errors from this conversion.
...
def mp3_process():
    ...
    subprocess.run(['yt-dlp.exe', '-x', '--audio-format', 'mp3', '--audio-quality', '0', video_link], check=True)
    
def mp3_metadata():
    video_link = entry.get()
    subprocess.run(['yt-dlp.exe', '-x', '--embed-metadata', '--embed-thumbnail', '--audio-format', 'mp3', '--audio-quality', '0', video_link], check=True)
    
def mp3_cookies():
    video_link = entry.get()
    subprocess.run(['yt-dlp.exe', '--cookies-from-browser', browser_cookies, '-x', '--audio-format', 'mp3', '--audio-quality', '0', video_link], check=True)    
    

    # flac
def flac_process():
    video_link = entry.get()
    subprocess.run(['yt-dlp.exe', '-x', '--audio-format', 'flac', '--audio-quality', '0', video_link], check=True)
    
def flac_metadata():
    video_link = entry.get()
    subprocess.run(['yt-dlp.exe', '-x', '--embed-metadata', '--embed-thumbnail', '--audio-format', 'flac', '--audio-quality', '0', video_link], check=True)
    
def flac_cookies():
    video_link = entry.get()
    subprocess.run(['yt-dlp.exe', '--cookies-from-browser', browser_cookies, '-x', '--audio-format', 'flac', '--audio-quality', '0', video_link], check=True)


    # mp4
def mp4_process():
    video_link = entry.get()
    subprocess.run(['yt-dlp.exe', '-S', 'ext:mp4:m4a', video_link], check=True)
    
def mp4_subs_process():
    video_link = entry.get()
    subprocess.run(['yt-dlp.exe', '-S', 'ext:mp4:m4a', '--all-subs' ,video_link], check=True)
    
def mp4_cookies_plus_subs():
    video_link = entry.get()
    subprocess.run(['yt-dlp.exe', '--cookies-from-browser', browser_cookies,'-S', 'ext:mp4:m4a', '--all-subs' ,video_link], check=True)

def mp4_cookies():
    video_link = entry.get()
    subprocess.run(['yt-dlp.exe', '--cookies-from-browser', browser_cookies, '-S', 'ext:mp4:m4a', video_link], check=True)


    # webm
def webm_process():
    video_link = entry.get()
    subprocess.run(['yt-dlp.exe', video_link], check=True)

def webm_cookies():
    video_link = entry.get()
    subprocess.run(['yt-dlp.exe', '--cookies-from-browser', browser_cookies, video_link], check=True)
...
def download_bin(): # downloads the yt-dlp binary from Github
    try:
        url.request.urlretrieve('https://github.coyt-dlp.exe/yt-dlp/releases/latest/download/yt-dlp.exe', "yt-dlp")
...
def about_info():
    tk.messagebox.showinfo(title='About yt-dlp simpleGUI',
                           message="This is a simple GUI for yt-dlp. \n"
                                   ""
                                   "Credits to github.coyt-dlp.exe \n"
                                   ""
                                   "Version: 0.0.3 \n"
                                   "made by emexis \n"
                           )
